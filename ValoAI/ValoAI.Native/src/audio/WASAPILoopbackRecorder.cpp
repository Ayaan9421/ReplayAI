...
bool WASAPILoopbackRecorder::start(double maxDurationSec) {
    ...
    m_captureStartTime = std::chrono::steady_clock::now();  // record start time
    m_running = true;
    m_client->Start();
...
bool WASAPILoopbackRecorder::saveSnapshot(const string& outPath,
    ...
    if (snapshot.empty()) {
        cout << "[WASAPILoopback] Buffer empty\n";
        return false;
    }

    // Trim chunks that were captured BEFORE the video window started
    // Each chunk's capturedAt tells us exactly when that audio arrived
    int trimmed = 0;
    ...
    cout << "[WASAPILoopback] Trimmed " << trimmed << " chunks to align with video window\n";
